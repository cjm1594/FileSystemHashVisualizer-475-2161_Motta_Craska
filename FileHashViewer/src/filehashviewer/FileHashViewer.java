...
public class FileHashViewer {
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException, IOException {
        ...
        System.out.println("Note: Any hashes supported by Java MessageDigest function can be used.");
        ...
        System.out.println("Please enter the letter of the drive to save to:");
        String fs = scan.next();
        System.out.println("Please enter the name of the computer being investigated:");
        String compName = scan.next();
        System.out.println("Please enter the drive being parsed:");
        String drive = scan.next();
        FileNode root = new FileNode();
        root.createNode(drive + ":", null); //Creates the initial node for the file system root.
        ...
        Path outFile = Paths.get(fs + ":\\FHV\\" + compName + "\\" + drive + "\\hashOutput.txt");
        Files.write(outFile, output, Charset.forName("UTF-8"));
        
        //Writes CSV representation to file.
        Path outFile2 = Paths.get(fs + ":\\FHV\\" + compName + "\\" + drive + "\\hashOutput.csv");
    ...
    public static void populateNode(FileNode node, ArrayList selections) throws NoSuchAlgorithmException, IOException{
        ...
        */
        ...
        System.out.println(node.currentDir + "\n");
    ...
    public static void hashFunction(FileNode node, ArrayList selections) throws FileNotFoundException, NoSuchAlgorithmException, IOException{
        ...
        for(int i = 0; i < selections.size(); i++){
            ...
            node.hashVals.add(dString + ": " +hashString.toString());
                ...
                node.hashVals.add(dString + ": File not found!");
            }
        }
    }
    ...
    public static void csvNodes(FileNode node, ArrayList<String> csvOutput){
        ...
        if(!(node.children.isEmpty())){
           /**
            * This block of replace commands attempts to eliminate instances of the . and .. directories
            * being accounted for in file creation, in addition to eliminating stray commas.
            */
